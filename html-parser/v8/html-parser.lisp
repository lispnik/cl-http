...
;;; Last edited by smishra on Mon Aug 11 13:34:27 1997
...
(defmacro define-html-parser-context (name arguments
  ...
  (let ((form-substs `((?exitp-var . ,exitp-var)
    ...
    (multiple-value-bind
	  (use-variables on-open-tag on-pcdata on-close-tag)
	(sort-forms-by-separators forms '(:use-variables :on-open-tag :on-pcdata :on-close-tag))
      (declare (dynamic-extent use-variables on-open-tag on-pcdata on-close-tag))

      `(defun ,name (parser ,.arguments
		     ,@(unless (member '&aux arguments) '(&aux))
		     ,.use-variables ,exitp-var ,exit-value-var)
	(labels ((,save-var (it &optional its-pd)
	  ...
	  (loop for pd in (parser-stack parser)
		do (execute-open-tag-forms (pd-instance pd) pd (pd-start-pos pd) (pd-end-pos pd)
                                           parser)
		when ,exitp-var
