;;; -*- Mode: Lisp -*-
;;; Last edited by smishra on Tue Oct 20 14:46:46 1998
...
;;; Implements a handler based callback mechanism on top of the
...
(eval-when (load compile eval)
  ...
  ;; This ought to be in CL. The Hyperspec defines it.
  
  (unless (fboundp 'constantly)
      ...
      (cond ((eql tag-strings t)
            ...
            (t (mapc #'(lambda (tag-name)
               ...
               (assert (or (null *tag-handlers*) (consp *tag-handlers*))
                       ...
                       "No other handlers may be specified with a universal specification")))
      `(let ((*tag-handlers*
	      ,(if (eql tag-strings t)
		   `(make-handler
		     :tags t
		     :test ,(or test '(constantly t))
		     :function ,handler-fn)
		   `(cons (make-handler
			   :tags (list . ,tag-tokens)
			   :with-attributes ,(cons 'list with-attr-tokens)
			   :without-attributes ,(cons 'list without-attr-tokens)
			   :test ,(or test '(constantly t))
			   :function ,handler-fn)
		     *tag-handlers*))))
