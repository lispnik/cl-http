...
;;; Last edited by smishra on Thu Sep  3 15:19:53 1998
...
(defun initialize-parse-dispatch-functions ()
  (setf (get-value #t"CDATA" :quoted-parser) #'parse-html-literal
	...
	(get-value #t"NAME" :quoted-parser) #'(lambda (parser)
						(parse-html-literal parser #'tokenize-name))
	(get-value #t"NAME" :unquoted-parser) #'(lambda (parser)
						  (parse-html-name parser #'tokenize-name))
	(get-value #t"NUMBER" :quoted-parser) #'(lambda (parser)
  ...
  (let* ((attr-defn (when tag-defn
    ...
    (cond (attr-val
	   ...
	   (parse-html-open-tag-attributes
	    parser tag-defn (acons (or attr-defn attribute-name) attr-val rattr-list)))
	   ...
	   (let ((next-char (next-html-char parser)))
	     ...
	     (cond ((null next-char)
		   ...
		   ((eql next-char #\=)
		    ...
		    (html-skip-whitespace parser)
		    (parse-html-attribute-value parser name-or-val tag-defn rattr-list))
		   ...
		   ((html-name-start-char-p next-char)
		    ...
		    (parse-html-open-tag-attributes
		     parser tag-defn
		     (acons (if tag-defn (tag-value-attribute name-or-val tag-defn) name-or-val)
			    name-or-val rattr-list)))
		   (t
		    ;; something else entirely unexpected, so signal error
		    #+ignore
		    (cerror "Close tag now"
			    "Unexpected character ~A while parsing tag opening" next-char)
		    (nreverse rattr-list)))))
	  (t
	   ;; something else entirely unexpected, so signal error
	   #+ignore
