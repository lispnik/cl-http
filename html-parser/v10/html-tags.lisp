...
;; all rights reserved.
;;
;; $Id: //depot/rev/lisp/html-parser-10.0-alpha/html-tags.lisp#1 $
...
;; Assumption: The operators :set, :and, :or and :sequence cannot interact
;; with *, +, ?
...
(defun substitute-html-entities (l)
  ...
  (cond ((null l) nil)
	((parameter-entity-name-p l) (substitute-html-entities 
				      (symbol-value l)))
	((atom l) l)
	((parameter-entity-name-p (car l))
	 (let ((entity-value (substitute-html-entities
			      (symbol-value (car l)))))
...
(defun make-single-html-element (name start-optional end-optional attributes
				 content inclusions exclusions)
  (let ((attribute-forms (make-html-attribute-forms 
			  (substitute-html-entities attributes)))
	(content (resolve-sgml-operators (substitute-html-entities content)))
	(inclusions (resolve-sgml-operators 
		     (substitute-html-entities inclusions)))
	(exclusions (resolve-sgml-operators 
		     (substitute-html-entities exclusions))))
    `((push (tokenize-name ,(symbol-name name)) *dtd-tags*)
...
(defun make-multiple-html-elements (names start-optional end-optional
				    attributes content inclusions exclusions)
  (loop
      for name in (cdr (resolve-sgml-operators 
			(substitute-html-entities names)))
      append (make-single-html-element name start-optional end-optional 
				       attributes content
