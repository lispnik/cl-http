...
;; all rights reserved.
;;
;; $Id: //depot/rev/lisp/html-parser-10.0-alpha/html-parser.lisp#1 $
...
(defmethod find-tag-container (parser (token html-name-token))
  ...
  (loop
      ...
      for tag-instance = (pd-instance pd)
      unless search-start do (setq search-start tag-instance)
      unless last-container do (setq last-container 
				 (and (valid-container-p tag-instance token)
				      tag-instance))
...
(defun open-tag (it start end parser)
  (let ((stack (parser-stack parser))
        (container (or (when (member (name it) *dtd-toplevel-tags*) t)
		    ...
		    (let ((next-context (caddr transition))
		      ...
		      `(,(cond ((eq switch-test t)
				`(eq ,last-context-var ',(car transition)))
			       ((or (symbolp switch-test) 
				    (stringp switch-test))
				`(and (eq ,last-context-var 
					  ',(car transition))
				      (find-method #'name nil
						   (list
						    (class-of ,last-value-var))
						   nil)
				...
				`(and (eq ,last-context-var ',(car transition))
					  ...
					  ,switch-test)))
			       ((atom switch-test)
				(error "Unrecognized test condition ~A"
				       switch-test))
				...
				`(and (eq ,last-context-var ',(car transition))
				      (eq ,last-value-var 
					  (progn ,(cdr switch-test)))))
			       (t (error "Unrecognized test condition ~A"
					 switch-test)))
			,(if (eq next-context :end)
			     `(values :end ,last-value-var)
			   `(multiple-value-bind
				(,last-value-var eofp)
				(,(car next-context) ,parser-var 
						     ,.(cdr next-context))
