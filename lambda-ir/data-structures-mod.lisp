...
(local-define-class hashed-array
		    hashed-array
		    ()
  access-hash  
  linear-structure  
...
(defun initialize-hashed-array (hashed-array &key in-access-hash in-linear-structure in-name-structure)
  (with-slots (access-hash linear-structure name-structure)
		hashed-array
      (setf access-hash (or in-access-hash (hash-table))
	    name-structure (or in-name-structure (make-adjustable-array))
	    linear-structure (or in-linear-structure (make-adjustable-array)))))

(defun make-hashed-array (&key access-hash name-structure linear-structure)
...
(defmethod access-hashed-array ((hashed-array hashed-array) (key number) &key array-access (if-does-not-exist :error))
  ...
  (with-slots (linear-structure name-structure)
	      hashed-array
...
(local-define-class numerological-hashed-array 
		    nil
		    (hashed-array)
  )
...
(local-define-class caching-mixin
		    cache
		    ()
  storage
  )
...
(local-define-class table-caching-object
		    nil
		    (caching-mixin)
  )

(local-define-class a-list-caching-object
		    nil
		    (caching-mixin)
  )
...
(local-define-class freshness-mixin
		    nil
		    ()
  freshness-storage
  )

(local-define-class table-caching-object-with-freshness
		    nil
		    (freshness-mixin table-caching-object)
  )

(local-define-class a-list-caching-object-with-freshness
		    nil
...
(defmethod initialize-cache :after ((caching-object table-caching-object-with-freshness))
  (with-slots (freshness-storage)
	      caching-object
    (setf freshness-storage (hash-table))))
...
(local-define-class tagged-array
		    t-array
...
(local-define-class hashed-tagged-array
		    h-t-array
		    (tagged-array)
  names
  )

(defgeneric initialize-tagged-array (tagged-object &key in-linear-object in-tags in-names)
  (:documentation "Sets up the tagged-array object."))

(defmethod initialize-tagged-array ((tagged-array tagged-array) &key in-linear-object in-tags in-names)
  (declare (ignore in-names))
  (with-slots (linear-object tags)
	      tagged-array
    (setf linear-object (or in-linear-object (make-adjustable-array))
	  tags (or in-tags (make-adjustable-array)))
    (initialize-cache tagged-array))
  tagged-array)

(defmethod initialize-tagged-array ((tagged-array hashed-tagged-array) &key in-linear-object in-tags in-names)
  (with-slots (linear-object tags names)
	      tagged-array
    (setf linear-object (or in-linear-object (make-adjustable-array))
	  names (or in-names (make-adjustable-array))
	  tags (or in-tags (make-adjustable-array)))
    (initialize-cache tagged-array))
  tagged-array)

(defun make-tagged-array (&key linear-object tags)
  (let ((tagged-array (%create-unnamed-object 'tagged-array)))
    (initialize-tagged-array tagged-array :in-linear-object linear-object :in-tags tags)
    tagged-array))

(defun make-hashed-tagged-array (&key linear-object tags names)
  (let ((tagged-array (%create-unnamed-object 'hashed-tagged-array)))
    (initialize-tagged-array tagged-array :in-linear-object linear-object :in-tags tags :in-names names)
    tagged-array))
    ...
    (let ((old-array (access-cache tagged-array tag-or-tags)))
	  ...
	  (progn
	    (add-to-hashed-array old-array 
				 (1- (length linear-object))
				 (aref names (1- (length linear-object))))
	    old-array)
...
(defmethod absolute-index ((tagged-array hashed-tagged-array) key &key (tag (default-tags)))
  (with-slots (names tags)
	      tagged-array
    (loop for n downfrom (1- (length names)) to 0
	  do (if (and (equalp key (aref names n)) (degenerate-member tag (aref tags n)))
