...
#+(or (and :excl :allegro-version>= (version>= 4 2)) :lcl4.2)
(defpackage "MAKE" (:nicknames "MK") (:use "COMMON-LISP" #+:lcl4.2 "LCL"))
...
(eval-when (compile load eval)
  (import *exports* #-(or :cltl2 :lispworks :lucid) "USER"
	            #+(or :cltl2 :lispworks :lucid) "COMMON-LISP-USER")
  (import *special-exports* #-(or :cltl2 :lispworks :lucid) "USER" 
	                    #+(or :cltl2 :lispworks :lucid) "COMMON-LISP-USER"))
#+(OR :CMU :CCL :ALLEGRO :EXCL :lispworks :symbolics)
(eval-when (compile load eval)
...
(defun component-full-pathname-i (component type &optional (version *version*)
  ...
  (let ((pathname
    ...
    (when (eq (lp:pathname-host-type pathname) :logical)
      (setf pathname (lp::logical-pathname pathname))
...
#+:lucid (shadow 'lcl::run-unix-program)
#-symbolics
