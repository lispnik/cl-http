...
<DOCUMENTATION>
 <DESCRIPTION>
  ...
  the former takes about half as long to execute.
  </DESCRIPTION>
 <COPYRIGHT HREF='defsystem.lisp|root().descendant(1,COPYRIGHT)'/>
 <CHRONOLOGY>
  <DELTA><DATE>19981017</DATE><AUTHOR>jaa</AUTHOR>
   removed spurious #xCA valued characters from the tables - must have come
   from the cut-and-paste from the html texts. they were read as whitespace in
   mcl, but allegro saw them as symbols</DELTA>
  <DELTA><DATE>19981027</DATE>
...
(defParameter *xml-namechar-table*
  '(xml-letter? xml-digit? #\. #\- #\_ #\: xml-combiningchar? xml-extender?))
       ...
       ,(if (symbolp table)
          ...
          `(or ,@(mapcar #'(lambda (constraint)
                             (etypecase constraint
                               ...
                               (cons `(<= ,(ensure-code (first constraint)) code
                                          ,(ensure-code (rest constraint))))))
                         table))))))
...
(defun expunge-character-tables
       ()
  (setf *xml-char-table* nil)
  (setf *xml-space-table* nil)
  (setf *xml-letter-table* nil)
  (setf *xml-digit-table* nil)
  (setf *xml-ideographic-table* nil)
  (setf *xml-combiningchar-table* nil)
  (setf *xml-extender-table* nil)
  (setf *xml-namechar-table* nil)
  (setf *xml-initial-namechar-table* nil)
  (setf *xml-attvalue-entitychar-table* nil)
  (setf *xml-chardata-entitychar-table* nil)
  (setf *xml-cdata-entitychar-table* nil))
...
(time (dotimes (x 1000) (xml-letter? #\a)))
