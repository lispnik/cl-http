...
(defMethod initialize-instance :after
  ...
  (dolist (element (element.content self))
    (etypecase element
      (simple-element (setf (element.context element) self))
      (string t))))
...
(defMethod element-append
  ...
  (setf (element.context child) element))
...
(defun element-read-type
       (stream)
  (peek-char #\< stream)
  (stream-tyi stream)
  (read-token stream))

(defun read-token-string
       (stream)
...
(defun read-token
       (stream)
  (intern-token (read-token-string stream)))
...
(defun pi-read
       (stream)
  ;; processing instructions are read for side-effect only and are
  ;; not bound
  (stream-tyi stream)
  (let ((tag (read-token stream))
        (attributes (read-attributes stream))
        (class nil))
    (unless (and (setf class (find-class tag))
                 (subtypep tag 'processing-instruction))
      (error "not a pi: ~s: ~s." tag attributes))
    (process-instruction (make-instance tag :attributes attributes)
                         stream)
    (element-read stream)))
...
(defun definition-read
       (stream &aux token)
  (stream-tyi stream)
  (setf token (read-token stream))
  (cond ((string-equal token "element")
         (element-definition-read stream))
        ((string-equal token "attribute")
         (attribute-definition-read stream))
        (t
         (skip-tag stream)))
  (element-read stream))
...
(defun simple-element
       (gi &rest content)
  (make-instance 'simple-element :name gi :content content))
...
(defparameter *test*
   (simple-element 'test
                   "asdf" (simple-element 'test2 "qwer" "<>")))
