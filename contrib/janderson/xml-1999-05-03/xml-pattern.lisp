...
&LT;!ATTLIST ELEMENT
 ...
 <DD>initial version supporting element instances,
...
;;;
;;; interface for other concrete classes

(defMethod xml-pattern.attributes ((node abstract-element-model)) nil)

(defMethod xml-pattern.relations
           ((node element-model))
  (element-declaration.model (element-model.content node)))
(defMethod xml-pattern.relations
           ((node model-group))
  (element-model.content node))

(defMethod xml-pattern.type
           ((node element-model))
  (element-model.name node))

(defMethod xml-pattern.occurrence ((node element-model))
  (element-model.occurrence node))

(defMethod xml-pattern.wildcard? ((node element-model)) nil)
...
#|
 ...
 the respective functions 
    ...
    the base implementation includes support for the <CODE>element</CODE>
    and <CODE>element-model</CODE> and <CODE>XML-PATTERN-ELEMENT</CODE> classes."))
 |#

(defClass xml-pattern-element (xml-pattern element)
  ()
...
(defMethod xml-pattern.attributes
  ...
  (or (element.get pattern 'xsl::attributes)
      (remove 'xsl::attributes (element.content pattern)
              :test-not #'eq
              :key #'xml-pattern.type)))

(defMethod xml-pattern.occurrence
           ((pattern xml-pattern-element))
  (element.get pattern 'xsl::occurrence))

(defMethod xml-pattern.type
           ((node xml-pattern-element))
  ;; if no type is specified, then the type should match.
  ;; if the element has a dtd definition, that will take precedence
  (or (element.get node *xsl-type-attribute*) t))

(defMethod xml-pattern.variable
           ((node xml-pattern-element))
  (element.get node *xsl-variable-attribute*))

(defMethod xml-pattern.wildcard?
           ((node xml-pattern-element))
  (eq (xml-pattern.type node) 'xsl::any))

(defMethod xml-pattern.attributes ((node element-declaration)) nil)
(defMethod xml-pattern.occurrence ((node element-declaration)) 1)
(defMethod xml-pattern.relations ((node element-declaration))
  (element-declaration.model node))
(defMethod xml-pattern.type ((node element-declaration))
  (element-declaration.name node))
(defMethod xml-pattern.variable ((node element-declaration)) nil)
...
(defMethod pattern&datum-expression
           (predicate (expression element-model-group))
  (apply #'pattern&datum-expression-op predicate
         (element-model.connector expression)
         (element-model.content expression)))

(defMethod pattern&datum-expression
           (predicate (expression dtd-element-model))
  (funcall predicate (xml-node.name (element-model.content expression))))
...
(defMethod deconstruct-pattern-attributes
           ((att attribute-node) rest)
  (values (attribute-node.name att) (attribute-node.value att) rest))

(defMethod deconstruct-pattern-attributes
           ((el element) rest)
  (values (element.get el 'xsl::name) (element.get el 'xsl::value) rest))
...
(defMethod pattern-attribute&datum->continue 
           (name value (datum element) succeed fail result)
  (pattern-attribute&datum->continue name value
                                     (element.get datum name)
                                     succeed
                                     fail result))
...
(defMethod pattern-attribute&datum->continue
  ...
  (if (funcall predicate (element.get datum name))
    (pattern-succeed name datum succeed fail result)
    (pattern-fail name datum succeed fail result
                'pattern-attribute&datum->continue)))
...
(defMethod pattern-collector-satisfied?
  ...
  (<= (if *validate* (pattern-collector.min collector)
          1)
      (length (pattern-collector.data collector))
      (if *validate* (pattern-collector.max collector)
...
(defun read-pattern-element
       (from &key ((:element-class *element-class*) 'xml-pattern-element)
                  ((:dtd *document-type*) *xsl-document-type*))
