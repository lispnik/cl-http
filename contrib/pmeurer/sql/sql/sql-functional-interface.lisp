...
(defparameter *trace-sql* nil
  "If set to a true value, the sql commands sent to the database are additionally
written to *TRACE-OUTPUT*, or, in the case *TRACE-SQL* is bound to an output stream,
to that stream.")
...
Returns the database object."
  (let ((db ;; check if connection of requested type already exists
    ...
    (cond ((or (not db) 
           ...
           (when (or make-default (null *default-database*))
             (setf *default-database* db))
...
If rollback is t (the default), rolls back uncommited transactions."
  (when (null database)
    (error "No database is connected or no default database specified."))
...
JOINS, USING and SET-OPERATION are not implemented yet" 
  (let ((database (or (cadr (member :database expressions)) *default-database*)))
    (if (null database)
        (error "No database is connected.")
      (let ((query-exp (apply #'make-select-expression database expressions))
