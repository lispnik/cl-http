...
(defmethod db-connect ((db odbc-database) db-name user-id password autocommit)
  (if user-id
    ...
    ;; does not work yet. Options are :prompt, :noprompt, :complete, :required
    (%sql-driver-connect (hdbc db) db-name :prompt #+ignore :prompt))
  ...
  (let ((completion (%sql-driver-connect 
    ...
    (multiple-value-bind (dsn uid pwd)
      ...
      (flet ((non-string-null (x) (and x (not (string= x "")) x)))
        (setf (sql::db-name db) (or (non-string-null (sql::db-name db)) dsn))
        (setf (sql::db-user-id db) (or (non-string-null (sql::db-user-id db)) uid))
        (setf (sql::db-password db) (or (non-string-null (sql::db-password db)) pwd)))))
  db)
  ...
  (when (query-active-p query)
    (with-slots (column-count column-data-ptrs column-c-types column-sql-types 
      ...
      (values
       ...
       #+ignore
       (progn
         ;;(when no-data (%free-statement hstmt :unbind)) ; ??
         (setf (query-active-p query) (not no-data)))))))
    ...
    (labels ((initialize-column (col-nr)
                (multiple-value-bind (name sql-type precision scale nullable-p)
                  ...
                  #|(let* ((c-type (sql-to-c-type sql-type))
                    ...
                    (vector-push-extend precision ; *** ??(if (zerop precision) nil precision)
                                        column-precisions)
      ...
      (loop for parameter in parameters
            ...
            (let* ((sql-type (aref column-sql-types parameter-nr))
                   ...
                   (parameter (case sql-type
                                (#.$SQL_DECIMAL 
                                 (if parameter
                                   (write-to-string parameter :base 10)
                                   "")) ; ??
                                (otherwise parameter)))
              ...
              (cond ((and (< column-size +max-precision+)
                     ...
                     (ecase lisp-type
                       ...
                       (:long 
                        ...
                        (setf out-len-ptr (%null-ptr)))
                       (:string
                        (%put-str data-ptr (or parameter "") precision)
                        (setf out-len-ptr (%null-ptr)))
                       (:binary
                                ...
                                (%sql-len-data-at-exec 
                                 (if sql-need-long-data-len 
                                   (if (consp parameter)
                                     ...
                                     #+ignore
                                     (length parameter)
                                     (min (print (length parameter)) +max-precision+)
                                     )
                                   0)))))))
      ...
      (loop for parameter in parameters
            ...
            (let* ((sql-type (aref column-sql-types parameter-nr))
              ...
              (cond ((and (< column-size +max-precision+)
                          (not (and (consp parameter) (eq (car parameter) :stream))))
                     (ecase lisp-type
                       (:short (%put-word data-ptr parameter))
                       (:long (%put-long data-ptr parameter))
                       (:string (%put-str data-ptr parameter precision)))
                     (%sql-bind-parameter hstmt parameter-nr $SQL_PARAM_INPUT
                                          (aref column-c-types parameter-nr)
                                          sql-type
                                          precision
                    ...
                    (t
                     ...
                     (%sql-bind-parameter hstmt parameter-nr $SQL_PARAM_INPUT
                                          (aref column-c-types parameter-nr)
                                          sql-type
                                          +max-precision+ ;= NT ; 0 ;; precision
                                          0 ;; scale; remark as above
                                          data-ptr ;; = rgbValue
                                          0; +max-precision+ ;0 = NT ;; if +max-precision+ on NT get error: no data pending
                                          out-len-ptr)
                     (print "after")
                     (%put-long out-len-ptr 
                                (%sql-len-data-at-exec 
