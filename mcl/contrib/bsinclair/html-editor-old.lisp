  ...
  (flet ((fn (x) 
    ...
    (mapcar #'(lambda (x) 
            ...
            `((:paragraph "Insert a paragraph break.~%HTML fills until it sees one.")
              (:title "Insert a title spec.~%Every file should start with one.")
...
(defparameter *html-commands*
  (flet ((anl (string) (format nil "~a~%" string))      ; Append New Line
    ...
    (mapcar #'(lambda (x)
            ...
            `((:paragraph "<p>" "" (:meta #\p) 
               "Paragraph <p> (m-p)")
              (:title "<title>" "</title>" (:meta :shift #\T)
               "Title <title> (m-sh-T)")
              ...
              (:break ,(anl "<br>") "" (:meta :shift #\R) "Line Break <br> (m-sh-R)")
              (:rule ,(anl "<hr>") "" nil "Horizontal Rule <hr>")
              (nil nil nil nil "-")
              ...
              (:styles nil nil nil ("Styles"
                                    ("Definition <dfn>" :dfn-style)
                                    ("Emphasis <em>" :em-style)
                                    ("Citation <cite>" :cite-style)
                                    ("Code <code>" :code-style)
                                    ("Keyboard <kbd>" :kbd-style)
                                    ("Status Msg <samp>" :samp-style)
                                    ("Strong <strong>" :strong-style)
                                    ("Variable <var>" :var-style)
                                    ("-" nil)
                                    ("Bold <b>" :b-style)
                                    ("Italic <i>" :i-style)
                                    ("Typewriter <tt>" :tt-style)))
              (:dfn-style "<dfn>" "</dfn>")
              (:em-style "<em>" "</em>")
              (:cite-style "<cite>" "</cite>")
              (:code-style "<code>" "</code>")
              (:kbd-style "<kbd>" "</kbd>")
              (:samp-style "<samp>" "</samp>")
              (:strong-style "<strong>" "</strong>")
              (:var-style "<var>" "</var>")
              (:b-style "<b>" "</b>")
              (:i-style "<i>" "</i>")
              (:tt-style "<tt>" "</tt>")
...
(defun define-html-fred-commands ()
  (dolist (command *html-commands*)
    (let ((name (html-command-name command))
      ...
      (when key
        (comtab-set-key
         (if (eq name :toggle-html-mode) *comtab* *HTML-comtab*)
         key
         #'(lambda (w) (do-html-command w name)))))))
...
(defun toggle-html-mode (w)
  ...
  (if (eq *HTML-comtab* (ccl::fred-comtab w))
    ...
    (progn
      ...
      (set-window-package w (find-package :html))
      (mini-buffer-update w)
...
(defparameter *space-width*
  (string-width " " *menu-font*))
...
(defun black-text (w)
  (when *colored-text-p*
    (multiple-value-bind (start end) (selection-range w)
      ...
      (buffer-set-font-spec (fred-buffer w) '((:color-index 0)) start end)))
  nil)                                  ; no insertion
...
(defun red-text (w)
  (when *colored-text-p*
    (multiple-value-bind (start end) (selection-range w)
      (when (eql start end)
        (setq start nil end nil))
      (buffer-set-font-spec (fred-buffer w) *command-font-spec* start end)))
  nil)                                  ; no insertion
    ...
    (let* ((buf (fred-buffer view))
      ...
      (flet ((do-the-undo-thing (string start)
        ...
        (when (eql start end)
          ...
          (when text-spec
            (buffer-set-font-spec buf text-spec)))))))
