...
;;; This package currently requires MCL-PPC, either 3.9 or 4.0 in order for timer
;;; processes to be automatically killed when the timer is GC'd.
;;; Other than that, it works in 3.0.
;;; In 3.0, you need to explicitly execute (ccl::terminate timer) to get rid
...
;;;
;;;  08/09/99 akh define reset-timer-relative
...
(defun get-decoded-universal-time ()
  (rlet ((dt :longDateRec))
    (multiple-value-bind (time-zone dst) (get-time-zone)
      ...
      (let ((time-high (%get-word (%int-to-ptr $Time)))
            (time-low (%get-word (%int-to-ptr $Time) 2))
