...
;; uncomment when installed in MCL -- JCMa 4/3/1999.
#+ignore 
...
;;;                 defparameter -> defvar
;;; 10/29/97 jcma,rj Fix terje's fix by added &allow-other-keys to initialize-instance for OT streams
...
(defun init-opentransport ()
   ...
   stream functions."
   (or *opentransport-initialized*
         ; Some utility functions - notably #_InitOpenTransport -
         ; aren't in the shared libraries that Apple distributes.
         ...
         (let* ((err nil))
            (multiple-value-bind (conn mainaddr error-number error-frag)
                ...
                (if (and conn (eql (setq err (#_InitOpenTransport)) #$kOTNoError))
...
(defun ot-make-stream-endpoint (stream &optional configptr) ; <<  
   (let* ((endpoint (if stream (opentransport-alloc-endpoint-from-freelist stream)))
      ...
      (when (null endpoint)
          ...
          (rlet ((errP :osstatus))
             (setq endpoint (#_OTOpenEndpoint configptr 0 (%null-ptr) errP)
         ...
         (multiple-value-bind (context endpoint) 
             ...
             (let* ((err #$kOTNoError)
                ...
                (macrolet ((check-ot-error-return (error-context)
                    ...
                    (setf (ot-conn-call conn) (#_OTAlloc endpoint #$T_CALL #$T_ADDR errP))
                    (check-ot-error-return :alloc)
                    (setf (ot-conn-bindreq conn) (#_OTAlloc endpoint #$T_BIND #$T_ADDR errP))
                    (check-ot-error-return :alloc)
                    (setf (ot-conn-bindret conn) (#_OTAlloc endpoint #$T_BIND #$T_ADDR errP))
                    (check-ot-error-return :alloc)
                    (setf (ot-conn-options conn) (#_OTAlloc endpoint #$T_OPTMGMT #$T_OPT errP))
                    (check-ot-error-return :alloc)
                    conn)))))) 
...
(defun ot-proxy-bind (context bindreq)
   (rlet ((errP :osstatus))
      (let* ((endpoint (pref context :ot-context.ref))
                (bindret (#_OTAlloc endpoint #$T_BIND #$T_ADDR errP))
...
(defun ot-tcp-proxy-bind (context endpoint host port reuse-p)    
   (rlet ((errP :osstatus))
      (let* ((bindreq (#_OTAlloc endpoint #$T_BIND #$T_ADDR errP))
                (bindret (#_OTAlloc endpoint #$T_BIND #$T_ADDR errP))
                (status (pref errP :osstatus)))
         ...
         (when (= status #$kOTNoError)
             (unwind-protect
                (progn
                   (ot-context-negotiate-4-byte-option context #$inet_ip #$ip_reuseaddr (if reuse-p #$T_YES #$T_NO))
...
(defun ot-new-inet-service ()
   (init-opentransport)
   (rlet ((errp :oSerr))
      (let* ((endpoint (#_OTOpenInternetServices (%int-to-ptr (logand #xffffffff #$kDefaultInternetServicesPath)) 0 errp))
                (err (%get-long errP)))
...
(defclass opentransport-binary-tcp-stream (#-:mactcp-compatibility opentransport-tcp-stream
...
#+:mactcp-compatibility
(defclass binary-tcp-stream (opentransport-tcp-stream)
   ())
...
(when (boundp '*sleep-wakeup-functions*)
    (pushnew #'opentransport-cleanup *sleep-wakeup-functions* :key #'function-name :test #'eq))
