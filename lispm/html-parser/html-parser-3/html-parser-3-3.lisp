...
;;; Patch file for HTML-PARSER version 3.3
;;; Reason: replace html-reader with new version.
;;; Written by JCMa, 8/23/96 19:37:13
...
;;; Experimental Working LispM Mailer 6.0, HTTP Server 60.16,
...
;;; Essential Image Substrate 427.0, HTML Parser 3.2, dtd compiler 3.0,
...
(SCT:FILES-PATCHED-IN-THIS-PATCH-FILE 
  "HTML-PARSER:HTML-PARSER;HTML-READER.LISP.7")


;========================
(SCT:BEGIN-PATCH-SECTION)
(SCT:PATCH-SECTION-SOURCE-FILE "HTML-PARSER:HTML-PARSER;HTML-READER.LISP.7")
...
;;; Last edited by smishra on Fri Aug 23 11:40:24 1996
...
(defun parse-html-open-tag ()
  ...
  (let* ((tag-name (parse-html-name  #'tokenize-name))
	 (tag-instance (and tag-name
			    (make-instance
			      'html-tag-instance
			      :instance-of (or (tag-definition tag-name)
					       (tag-definition "UNKNOWN"))))))
    (when tag-instance
    ...
    (and (or (char= literal-start-char #\")	;" -> just to make emacs happy
	 ...
	 (do ((start-pos *current-pos*)
	     ...
	     ((eql char literal-start-char)
	      (funcall processing-fn *html-string* start-pos (1- *current-pos*)))))))
